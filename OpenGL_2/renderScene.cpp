...
void InitScene(LPVOID lpParam)
{
	...
	string sTextureNames[] = { "grass.jpg", "brick.jpg", "log.jpg", "bookshelf.jpg", "oak.jpg", "cobblestone.jpg", "command.jpg" };

	FOR(i, NUMTEXTURES)
	{
		tTextures[i].LoadTexture2D("data\\textures\\task\\" + sTextureNames[i], true);
	...
	sbMainSkybox.LoadSkybox("data\\skyboxes\\task\\", "sky_ft.tga", "sky_bk.tga", "sky_lf.tga", "sky_rt.tga", "sky_up.tga", "sky_dn.tga");
...
void RenderScene(LPVOID lpParam)
{
	...
	glm::vec3 vPos2 = glm::vec3(30.0f, 15.0f, 80.0f);
	mModelMatrix = glm::mat4(1.0f);
	mModelMatrix = glm::translate(mModelMatrix, vPos2);
	//mModelMatrix = glm::rotate(mModelMatrix, fGlobalAngle / 4, glm::vec3(1.0f, 1.0f, 1.0f));
	...
	FOR(i, 6)
	{
		...
		tTextures[i + 1].BindTexture(1);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, (GLvoid*)(sizeof(int) * i * 6));
	}


	// disable cull and multitexturing
	...
	vPos = glm::vec3(0.0f, 10.0f, 0.0f);
	mModelMatrix = glm::translate(glm::mat4(1.0), vPos);
	mModelMatrix = glm::rotate(mModelMatrix, PI / 2, glm::vec3(1.0f, 0.0f, 0.0f));
	mModelMatrix = glm::rotate(mModelMatrix, fGlobalAngle, glm::vec3(0.0f, 0.0f, 1.0f));

	translate = abs(10 - fmod(fGlobalAngle, 20));
	mModelMatrix = glm::translate(mModelMatrix, glm::vec3(translate, 0.0f, 0.0f));
	mModelMatrix = glm::rotate(mModelMatrix, 2 * fGlobalAngle, glm::vec3(0.0f, 0.0f, -1.0f));

	spMain.SetUniform("matrices.normalMatrix", glm::transpose(glm::inverse(mModelMatrix)));
	spMain.SetUniform("matrices.modelMatrix", &mModelMatrix);
	glDrawArrays(GL_TRIANGLES, 6, iTorusFaces1 * 3);
	...
	spMain.SetUniform("matrices.normalMatrix", glm::transpose(glm::inverse(mModelMatrix)));
	spMain.SetUniform("matrices.modelMatrix", &mModelMatrix);
	glDrawArrays(GL_TRIANGLES, 6 + iTorusFaces1 * 3, iTorusFaces2 * 3);
	...
	mModelMatrix = glm::translate(glm::mat4(1.0), vPos);
	mModelMatrix = glm::scale(mModelMatrix, glm::vec3(1 + translate / 10, 1 + translate / 10, 1 + translate / 10));
	spMain.SetUniform("matrices.normalMatrix", glm::transpose(glm::inverse(mModelMatrix)));
	spMain.SetUniform("matrices.modelMatrix", &mModelMatrix);
	glDrawArrays(GL_TRIANGLES, 6 + iTorusFaces1 * 3 + iTorusFaces2 * 3, iCylinderFaces1 * 3);
