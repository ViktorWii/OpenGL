...
bool COpenGLWinApp::CreateAppWindow(string sTitle)
{
	...
	if (!oglControl.InitOpenGL(hInstance, &hWnd, InitScene, RenderScene, NULL, &oglControl))
		return false;
...
{
	if (!appMain.InitializeApp("Home Work 1"))
		return 0;
	appMain.RegisterAppClass(hInstance);

	if (!appMain.CreateAppWindow("OpenGL. Home Work"))
		return 0;
	appMain.ResetTimer();
